...
A self-hosted approval inbox for [OpenBao control groups](https://openbao.org/community/rfcs/control-groups/). Requesters submit one or more wrapping accessors with a shared reason, approvers review the fixed group in an installable React PWA, and each authorization is recorded with the logged-in human's OpenBao identity.
...
Three OpenBao credentials have deliberately different roles:

- **Service token:** inspects explicitly submitted accessors through `sys/control-group/request`, may revoke a stored wrapping token after an authenticated approver rejects its group, and may read explicitly configured non-secret approval metadata. It cannot list accessors, approve, unwrap, or renew. Context-reader policies must expose only data safe for every approver to inspect.
- **Requester token:** sent by `bao-cred` only when registering a group. The server calls `lookup-self`, verifies every submitted request belongs to the same identity entity, then discards the token. It is never persisted and is never used to read or unwrap secrets.
...
Accessors, reasons, deferred request payloads, and push subscriptions are AES-256-GCM encrypted before SQLite persistence; the database is created with mode `0600`. Payloads are omitted from API responses unless `requests.expose_data = true` is explicitly configured; reasons remain visible to approvers. Logs, SSE messages, push payloads, URLs, and browser storage do not contain accessors or tokens. Push notifications contain only a generic “approval pending” message, are sent only for an active eligible approver session, expire with that session, and are restricted to configured public push-service host suffixes.

## Architecture

```text
Requester ── reads ──> OpenBao ── wrapping accessors ──┐
    │                                                  │
    └── authenticated fixed-group submission ──> Go HTTP API ──> encrypted SQLite
                                                    │                 │
OpenBao <── human authorize / app revoke ───────────┤── SSE ──────────┤
                                                    └── Web Push      │
Browser PWA ── HttpOnly session + CSRF ───────────────────────────────┘
```
...
Install [`config/service-policy.hcl`](config/service-policy.hcl) on a dedicated orphan machine token created **without the default policy**. Assign [`config/approver-policy.hcl`](config/approver-policy.hcl) to the human identity group that appears in your protected path's `control_group` factor.
...
Then create the service token with both policies:

```sh
bao token create -orphan \
  -policy=openbao-authorizer-service \
  -policy=openbao-authorizer-github-context \
  -no-default-policy \
  -renewable=false
```

The service policy uses path-scoped `sudo`; it is not a root token. `auth/token/revoke-accessor` can revoke any token whose accessor is supplied, so this credential must be tightly controlled. The application invokes it only for an encrypted accessor that was authenticated, reviewed, and stored as part of an explicit submission. The `-no-default-policy` flag is required: without it, OpenBao attaches capabilities such as token self-renewal that are outside the documented service role.
...
Create mode-`0400` files for the application encryption key and service token,
...
chmod 0400 /run/secrets/openbao-authorizer-key /run/secrets/openbao-authorizer-service-token
...
`bao-cred` reads one path or an aliased set of paths. When OpenBao applies control groups, it submits all resulting accessors to the authorizer as one fixed group, retries transient submission failures with the same idempotency key, waits up to 15 minutes, then consumes each approved wrapping token once. Set `BAO_AUTHORIZER_ADDR` or pass `-authorizer-address`; use `-reason` for the shared justification. The CLI honors standard OpenBao variables such as `BAO_ADDR`, `BAO_NAMESPACE`, and `BAO_CACERT`.
...
# One approval group for several paths. JSON and templates receive an object
# keyed by alias; mappings start with that alias.
bao-cred -request db=database/creds/app \
  -request gh=github/token/project-example \
  -reason 'Run the development environment' \
  -format dotenv -map DB_USER=db.username -map GH_TOKEN=gh.token
```
...
operation. Multi-request operations are strict by default: they emit nothing and do not launch a child command unless every requested value succeeds. `-allow-partial` explicitly permits available values and warns about omissions. Use `-quiet`, `-timeout`, and `-poll-interval` for automation. Shell
...
required block. Encryption keys, service tokens, and optional VAPID keys are
read from files so secret values do not appear in HCL or the process
environment.

`requests.require_reason` is global. When enabled, submissions must contain a trimmed, non-empty reason of at most 500 characters. When disabled, the same field is optional but is still retained and displayed when supplied. `reconciliation.interval` controls how often the server refreshes only already-submitted pending members.
...
The service token must have `read` access to every configured `read_path`.
...
Groups move through `pending`, `approved`, `rejected`, `expired`, `approval_failed`, or `rejection_failed` states. Failed decisions remain retryable and expose each member's actual native state. The reconciler checks only submitted accessors and marks the whole group expired when a required member disappears.
...
- `POST /api/v1/request-groups` — submit an ordered accessor set, reason, and idempotency key using `X-Vault-Token`
- `GET /api/v1/request-groups` — list fixed groups without accessors
- `GET /api/v1/request-groups/{id}` — retrieve one fixed group without accessors
- `POST /api/v1/request-groups/{id}/approve` — authorize every pending member in order with the session's human token
- `POST /api/v1/request-groups/{id}/reject` — revoke every member and record the aggregate result
...
- Only explicitly submitted accessors are visible. Clients that do not call the submission API cannot be approved through this application.
